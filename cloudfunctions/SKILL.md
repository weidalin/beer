...
> **仓库副本：** 从 `~/.agents/skills/cloudbase/SKILL.md` 同步至本目录 `SKILL.md`（v2.19.2）。`references/` 未打包，见正文 **cnb.cool** 上游链接；Cursor 全局技能仍以 `~/.agents/skills/cloudbase/` 为准（未删除原安装）。
>
> **本仓库云函数：** `wxLogin/`、`init_db/` 各含 `index.js` + `package.json`（与微信云开发约定一致）。控制台本地上传 zip：在仓库根执行 `node cloudfunctions/zip-cloudfunctions.mjs`，产物在 `cloudfunctions/dist-zips/*.zip`。
